...
object Cardioid extends App {
  ...
  def createImages() = {
    apply(ViewportUtil.start, (view, color) => s"images/start_$color.png")
  ...
  def checkNewton() = {
    ...
    for(t <- 2d until 3 by 0.001)
      println(s"$t, ${card.dist(t, x0, y0)}, ${card.d_derived(t, x0, y0)}, ${card.d_derived2(t, x0, y0)}")
  }
  ...
  def checkAbls() = {
    ...
    val check = (for{
      ...
      y <- -2d to 2d by 0.1
      t <- -2d to 2d by 0.1
      d = card.der1DivDer2(t, x, y)
      d1c = card.d_derived_ana(t, x, y)
      d2c = card.d_derived2_ana(t, x, y)
    } yield {
