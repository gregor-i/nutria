...
vec2 complex_product(const in vec2 a, const in vec2 b){
 return vec2(a.x*b.x-a.y*b.y, a.x*b.y+a.y*b.x);
}

vec2 complex_divide(const in vec2 a, const in vec2 b){
  return vec2(((a.x*b.x+a.y*b.y)/(b.x*b.x+b.y*b.y)),((a.y*b.x-a.x*b.y)/(b.x*b.x+b.y*b.y)));
}

// see: http://mathworld.wolfram.com/ComplexExponentiation.html
vec2 complex_power(const in vec2 a, const in vec2 b){
...
vec2 complex_exp(const in vec2 z){
 return exp(z.x) * vec2(cos(z.y), sin(z.y));
}

vec2 complex_log(const in vec2 a){
...
vec2 complex_sqrt(const in vec2 z){
...
vec2 complex_asin(const in vec2 z){
  vec2 z2 = complex_product(z, z);
  vec2 s = complex_sqrt(vec2(1.0 - z2.x, -z2.y));
...
vec2 complex_atan(const in vec2 z){
