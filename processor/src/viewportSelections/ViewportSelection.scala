...
object ViewportSelection {
  ...
  val selection = ViewportImporter(Source.fromFile("resources/selection.json").mkString).getOrElse(throw new AssertionError("collection was not decoded"))

  val focusIteration1 = ViewportImporter(Source.fromFile("resources/focus1.json").mkString).getOrElse(throw new AssertionError("collection was not decoded"))

  val focusIteration2 = ViewportImporter(Source.fromFile("resources/focus2.json").mkString).getOrElse(throw new AssertionError("collection was not decoded"))
}